...
- 手動修正→辞書登録の確認を、OS標準の confirm() から**字幕直下のインラインバー**（登録する／登録しない）に変更。どちらを選んでも修正テキストには一切触れず、スクロール位置も動かない
- 「大きな変更」とみなす閾値を13文字→10文字に変更（10文字以上の差分は文の書き換えとみなし提案しない）

## v7 の改善点（2026-07-13）

- **辞書を1本に統合**: 「単語＋読みがな」「直接補正ルール」「組み込みルール」の3層を廃止し、単一の「誤変換・読みがな→正しい語」ルール集（localStorage `kendoRules`）に統合。組み込み47ルールは初回起動時にユーザー辞書へ取り込まれ、以後すべて追加・編集・削除可能。旧形式の登録データは自動移行
- かなだけのルール（3文字以上）はひらがな・カタカナ両表記に自動対応（2文字以下は誤爆防止のため登録した表記のみ。例: ツキ→突き は「気がつき」を壊さない）
- 同じ誤変換を再登録すると古いルールを置き換え（重複しない）
- 字幕・録音の時間表示から小数を削除（m:ss形式）
- コメント音量スライダーの上限を200%→300%に拡大

## v8 の改善点（2026-07-13）

- 音声認識の失敗理由を画面に表示（従来は not-allowed 系以外を無言で握りつぶしていた）。network / audio-capture / service-not-allowed など種別ごとに日本語で対処を案内。no-speech（無音）は無視。start() の例外も表示
- 別端末で「録音はできるが字幕が出ない」問題の切り分け用（多くは iOS「設定→一般→キーボード→音声入力」オフが原因）
